...
describe('TownServiceApiSocket', () => {
  ...
  it('Dispatches town messages to all clients in the same town, including the client who sent the message', async () => {
    ...
    const { messageReceived: messageReceived3 } = TestUtils.createSocketClient(
      server,
      joinData3.coveySessionToken,
      town.coveyTownID,
    );
  ...
  it('Dispatches proximity messages to all nearby clients in the same town, including the client who sent the message', async () => {
    ...
    const { socket: movementSocket } = TestUtils.createSocketClient(
      server,
      joinData.coveySessionToken,
      town.coveyTownID,
    );
    const { socket: messageSocket, messageReceived, playerMoved } = TestUtils.createSocketClient(
      server,
      joinData2.coveySessionToken,
      town.coveyTownID,
    );
    const {
      messageReceived: messageReceived2,
      playerMoved: playerMoved2,
  ...
  it('Dispatches direct messages to the specified client in the same town, including the client who sent the message', async () => {
    ...
    const message = TestUtils.createDirectMessageForTesting(
      joinData2.currentPlayers[0]._id,
      joinData2.currentPlayers[1]._id,
    );
